...
def setup_argparse() -> ArgumentParser:
    ...
    parser.add_argument(
        "-t", "--type", type=str, required=True, choices=["synthetic", "original"]
    )
    parser.add_argument(
        "-p",
        "--orig-path",
...
def save(mat, num, args, sparsity=None):
    ...
    if args.type == "original":
        ...
        if not os.path.exists(base_path):
            os.mkdir(base_path)
        path = os.path.join(
            base_path, "_".join([str(x) for x in [args.r, args.c, num]]) + ".npz"
        )
    else:
        ...
        if args.ep is not None:
            ...
            path = os.path.join(
                base_path,
                "_".join(
                    [str(x) for x in [args.r, args.c, args.mu, args.si, sparsity, num]]
                )
                + ".npz",
            )
...
def original(args):
    ...
    for run in tqdm(range(args.num_files)):
        row_start, col_start = (
            default_random.choice(row_start_range),
            default_random.choice(col_start_range),
        )
        submat = mat[
            row_start : row_start + sample_shape[0],
            col_start : col_start + sample_shape[1],
        ]
...
def synthetic_condition(args):
    assert args.r == args.c
    "Not sure how to control condition number for non-square matrix"
    pts = np.array(list(product(list(range(args.r)), list(range(args.c)))))

    m = args.r
    epsilon = (
        args.ep - 1
    )  # I'm observing for some reason that the condition number generated is one more than epsilon??
